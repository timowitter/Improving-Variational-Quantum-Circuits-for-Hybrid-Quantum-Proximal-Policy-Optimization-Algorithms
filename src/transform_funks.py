...
def transform_obs_to_binary(observation, n_qubits):
    ...
    for i in range(n_qubits):
            temp = str(bin(int(observation)))[2+i:3+i]
            if temp == '':
                binary_observation[i] = 0.0
            else:
                binary_observation[i] = float(temp)
    return binary_observation

#args.gym_id, envs.single_observation_space.n
def trans_obs(observation, gym_id, single_observation_space_n):
    if (gym_id == "FrozenLake-v0" or gym_id == "FrozenLake-v1" or gym_id == "Deterministic-ShortestPath-4x4-FrozenLake-v0"):
...
def normalize_obs(observation, gym_id, n_qubits):
    if ((gym_id == "CartPole-v0" or gym_id == "CartPole-v1") and n_qubits == 4):   #specific normalisation for cartpole      terminates if |position|>2.4 or |angle|>0.2095
        norm_obs = observation
        norm_obs = torch.Tensor([torch.clamp((norm_obs[0]/4.8), -1.0, 1.0), torch.tanh(norm_obs[1]), torch.clamp((norm_obs[2]/0.418), -1.0, 1.0), torch.tanh(norm_obs[3])])  # 
    elif(gym_id == "FrozenLake-v0" or gym_id == "FrozenLake-v1" or gym_id == "Deterministic-ShortestPath-4x4-FrozenLake-v0"):
